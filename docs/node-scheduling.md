...
2. 填写节点名称、IPv4 地址、IPv6 地址和 Agent HTTPS 端口。地址必须是节点自身的公网 IP（纯 IP，不带端口、不写域名）；IPv4 和 IPv6 可以只填一个，也可以同时填写。
   “DNS 发布”默认为“自动（双栈同时发布）”：只有 IPv4 时发布 A，只有 IPv6 时发布 AAAA，双栈节点同时发布 A/AAAA。需要手动限制地址族时，选择“仅 IPv4”或“仅 IPv6”；如果所选地址族没有可用地址，站点会保持等待并在调度页面显示原因。
   留空也可以：节点首次注册时，Controller 会把观测到的注册来源 IP 自动填入，并在节点卡片上标注“自动探测，请核对”。该推断仅在来源 IP 全局可路由时生效，且不会覆盖已填写的地址；核对或修改并在面板保存后，标记转为人工确认。
