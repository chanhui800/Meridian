...
2. 填写节点名称、IPv4 地址、IPv6 地址和 Agent HTTPS 端口。**填哪个族就发布哪条记录**：
   - 只填 IPv4 → 站点只发布一条 `A` 记录；
   - 只填 IPv6 → 只发布一条 `AAAA` 记录；
   - 两个都填 → 同时发布 `A` 和 `AAAA`（双栈机默认行为），客户端按自身网络选择，IPv6 优先；
   - “DNS 解析”选择“仅 IPv4 / 仅 IPv6”可以把双栈机**限定**为只解析其中一个族；该选项不会凭空发布节点没有的族。

   地址必须是节点自身的公网 IP 字面量：纯 IP，不带端口、不写域名，IPv6 也不要加方括号。
   留空也可以：节点首次注册时，Controller 会把观测到的注册来源 IP 自动填入；此外 Agent 会上报自己网卡上的全局可路由地址，Controller **逐个健康探测**，只有探测通过的才会自动填入空着的族，并在节点卡片上标注“自动探测，请核对”。两种自动路径都只在槽位为空时生效，绝不覆盖已填写的地址；在面板保存后标记转为人工确认。
3. 设置流量上限、每月重置日、计费方式和优先级。流量上限填 `0` 表示不限额，重置日填 `0` 表示不自动重置（此时上限是**累计**上限）。
4. Controller 地址填写当前主控的完整 HTTPS 地址，例如 `https://panel.example.com:9090`。
5. 保存后，面板会显示一次性注册令牌和一键安装命令。只在目标节点上执行该命令。

### 双栈节点的注意事项

- **发布几个族，就探测几个族。** 调度器在发布 DNS 之前会对每个待发布的地址各做一次健康检查（用站点域名做 SNI 与证书校验，直连该字面量 IP）。任何一族探测失败，站点就停在 `dns=waiting` 并显示具体原因，不会写出一个连不上的记录。
- **Agent 监听是双栈的。** Agent 用 `net.Listen("tcp", ":端口")` 监听，Linux 默认 `net.ipv6.bindv6only=0` 时同一个 socket 同时接受 IPv4 与 IPv6。若该 sysctl 被设为 `1`，IPv4 客户端会连不上——此时 IPv4 那一族的健康探测会失败，调度器不会发布 `A` 记录，而不是发出一个坏记录。
- **切换族是先建后删。** 从单栈改成双栈、或从双栈限定为单栈时，调度器先写好要保留的记录、再删除不再发布的那一族，任一时刻主机名至少有一条可用地址；任何一步失败都会把整组记录恢复成改动前的状态。
- **一个族只有一条记录。** 同一个主机名上如果存在非 Meridian 标记的同名 `A`/`AAAA` 记录，调度器会拒绝写入并报“an untracked exact A/AAAA record already exists”，需要先自行清理那条记录。
- **手工改地址记得核对发布结果。** 保存后可以用 `dig A 站点域名` 与 `dig AAAA 站点域名` 确认实际发布了几条、指向哪里。

### 节点已经属于另一个主控时

注册令牌是绑定主控的一次性凭据，节点本地会保存所属主控的 `state.json`。在同一台机器上执行第二个主控的安装命令时，安装器会先用本地凭据探测该主控：

- 凭据被接受 → 该节点确实属于当前主控，按普通重装处理，保留原有身份。
- 凭据被拒绝（401/403）→ 安装器**明确报错并退出**，说明该节点已属于另一个主控，并打印带 `--reenroll` 的完整命令。它不会静默跳过注册，也不会报告成功。

这种情况下必须使用面板“重新生成脚本”给出的命令（其中包含 `--reenroll`），或自行补上该参数：

```bash
curl --proto '=https' --proto-redir '=https' --tlsv1.2 -fsSL \
  'https://panel.example.com:9090/api/agent/install.sh' | sudo bash -s -- \
  -e 'https://panel.example.com:9090' -t 'NEW_ONE_TIME_TOKEN' --reenroll
```

`--reenroll` 会清除本地旧身份并强制重新交换令牌，注册成功后一次性令牌即被消费。注意重新注册会让该节点在主控上获得新的 node GUID，旧 GUID 的流量基线与调度关联需要另行清理。
...
Agent 每 5 秒上报一次完整心跳和累计流量，另以 2 秒间隔发送只读实时流量采样。媒体库、观察记录、事件和完整站点状态仍在 5 秒完整报告中处理。Controller 对完整报告、实时采样和 WebSocket 共用每节点速率限制和单请求并发限制；正常周期不会触发限制。越权报告会被丢弃并计入安全审计，不会进入站点数据库。
