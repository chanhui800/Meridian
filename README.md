...
Docker 主控在已配置 Cloudflare ACME 凭据后，可执行 `docker exec meridian /app/meridian admin issue-edge-certificate` 为节点签发独立于面板私钥的 wildcard Edge 证书；证书缺失时 Agent 配置会保持等待并在调度页面显示最后一次错误。
