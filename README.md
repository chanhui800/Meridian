...
本仓库基于 [snnabb/Meridian](https://github.com/snnabb/Meridian) 修改，界面与部分交互参考了 [CF-EMBY-PROXY-UI](https://github.com/axuitomo/CF-EMBY-PROXY-UI)。当前正式版本为 `v1.8.39`。

## 这次更新了什么

`v1.8.39` 主要处理面板证书和实时状态：

- 泛域名证书进入到期前 30 天后自动续签，后台每 12 小时检查一次。
- 证书已经过期时，Meridian 会关闭面板 HTTPS、保留原证书文件并改用 HTTP 重启，避免管理面板彻底无法访问。
- ACME 邮箱和 DNS API Token 可在已登录的 TLS 页面中直接查看。Token 使用 AES-GCM 加密保存，也能随可选 TLS 备份安全迁移。
- 仪表盘站点列表增加实时下载、上传速度。HTTP、缓存命中、限速响应和 WebSocket 流量使用同一套累计计数，不再因为分钟落库或页面刷新短暂变成横杠。
...
- 提供双向流量统计、实时网速、请求日志、最终后端地址和 Telegram 定时日报。
...
仪表盘显示各站点的实时下载与上传速度。流量统计按进入 Meridian 的请求体和返回给客户端的响应体分别累计，因此总流量是双向流量。
