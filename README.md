...
域名入口需要在“全局设置 → TLS 设置”分别配置面板完整域名（例如 `panel.example.com`）和节点泛域名（例如 `*.edge.example.com`），再填写 Cloudflare DNS API Token。两者必须属于同一注册域但不能相同；面板证书只签面板域名，节点证书由各节点独立签发。证书由 ACME 申请和续签，Token 加密保存在数据库中。
