...
本仓库基于 [snnabb/Meridian](https://github.com/snnabb/Meridian) 修改，界面与部分交互参考了 [CF-EMBY-PROXY-UI](https://github.com/axuitomo/CF-EMBY-PROXY-UI)。当前正式版本为 `v1.8.47`。

## 这次更新了什么

`v1.8.47` 完善观看历史、TMDB 异步资料补全和 Hills 等客户端的表单/非 JSON 播放状态同步；详情浮窗按视口宽高与浏览器缩放自适应，在主内容区居中，背景由上至下平滑渐进，剧照与海报支持点击放大；站点保号提醒、5 次播放状态同步重置、媒体库数量和 Telegram 日报统一呈现。面板与官网继续支持跟随设备自动切换浅色/深色主题。

- 面板和官网默认根据设备深浅色偏好自动切换，并在设备主题变化时即时同步。
- 主题按钮支持“跟随设备、浅色、深色”三态循环；手动选择会保留为当前设备的明确偏好。
- 站点可选填写保号周期；同一播放身份完成 5 次有效播放状态同步后自动重置倒计时，临期状态使用红色文字提醒。
- Telegram 日报会列出已配置站点的当日保号完成状态或剩余天数。
- 面板从客户端已授权的 `/Items/Counts` 响应安全更新电影、剧集和集数量，不保存或重放客户端 Token。
- 站点可独立开启观看历史；仅从成功的播放状态同步中旁路提取媒体条目和进度，不保存用户名、访问令牌、客户端 IP、原始设备 ID、原始播放会话 ID 或请求正文。
- 未配置 TMDB 时仍可查看本地标题与观看进度；配置全局 Read Access Token 后，后台才会异步补全高置信匹配的海报与影片资料，失败不会影响代理或播放。
...
在站点编辑的“高级设置”中开启“观看历史记录”后，Meridian 会从后续成功的 Emby / Jellyfin 播放状态同步中生成站点级观看记录。功能默认关闭，不会回填旧日志，也不会保存用户名、令牌、客户端 IP、原始 DeviceId、原始 PlaySessionId 或完整请求正文。
...
观看历史是按站点独立开启的可选模块。成功的播放状态同步才会写入记录，同一站点的同一剧集会合并为一张卡片，优先显示最新未看完的一集，并展示观看进度、已观看时长、播放方式和站点名称。记录可以点击查看简介、上映时间、类型、评分、集数、更新信息、演员和剧照；移动端支持横向浏览演员与图片预览，长按卡片可直接删除。
