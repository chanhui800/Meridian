...
本仓库基于 [snnabb/Meridian](https://github.com/snnabb/Meridian) 修改，界面与部分交互参考了 [CF-EMBY-PROXY-UI](https://github.com/axuitomo/CF-EMBY-PROXY-UI)。当前正式版本为 `v1.8.41`，基于正式版 `v1.8.40`。

## 这次更新了什么

`v1.8.41` 在 `v1.8.40` 的仪表盘趋势与流量计费基础上补充：

- 仪表盘新增速度、请求和流量三张趋势图，可查看全部站点或单个站点，并切换实时、1 小时、6 小时、1 天和 7 天范围。
- 实时趋势按网络请求持续采样，X 轴会根据数据密度自动调整。桌面端和移动端都能用指针选中数据点，浮窗会自动避让页面边缘。
- 趋势卡片分别显示峰值速度、请求总数和计费流量总数；Y 轴固定六个分段，零数据不会再出现负数或异常单位。
- 全局设置新增单向、双向流量计费。双向模式按源站到 VPS 的入站流量加 VPS 到客户端的出站流量计算；单向模式只计算出站流量。
- 流量周期支持不重置或每月指定日期重置，29 至 31 日遇到短月会使用当月最后一天。站点额度、仪表盘已用流量和趋势统计使用同一计费口径。
- 仪表盘补充最近日志写入时间和 Telegram 日报任务名称，站点实时状态统一放在页面底部。
- 趋势图支持自定义开始/结束日期时间（精确到分钟），并遵循全局调度时区。
- 趋势图时间范围新增“本月”，按自然月 1 日 00:00 至当前时间统计，不受流量计费重置日设置影响。
- 趋势图悬浮卡片按选择范围显示站点名称及对应数据：全部站点逐项列出，单站点只显示当前站点。
