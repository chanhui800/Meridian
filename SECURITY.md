...
- 首次创建管理员必须提供 `SETUP_TOKEN`。原生一键安装器会生成随机令牌并写入受保护的 `.env`，且不会将其写入服务日志；官方 Docker 入口会生成随机令牌、写入数据卷内权限为 `0600` 的 `.meridian-secrets`，并仅在首次生成时输出到容器日志，方便用户完成初始化。创建成功后应用会从进程内存清除并忽略该令牌
...
- 功能按站点显式启用；Safe 只允许 `redirect`、`playback_info`，Compatible/Extreme 才可再选择 `hls`、`dash`。Safe/Compatible 不扫描未知 JSON 字段；Extreme 只在 PlaybackInfo 根和 MediaSource 子树内识别“整个字符串就是绝对 HTTP(S) URL”的值，不扫描 key、prose 子串、HTML、JavaScript、RequiredHttpHeaders 值或任意响应，也不接受客户端提供的 target
...
- Profile 网络边界：Safe 只启用 30x/PlaybackInfo，且仅 HTTPS:443，3 跳、256 authority、16 个 DNS 回答、每分钟 60 个新 authority、32 条动态流；Compatible 可启用 HLS/DASH，为 HTTP(S) 任意端口，5/1024/32/300/128；Extreme 可启用全部已支持结构化来源，为 10/4096/64/1200/512。三档协议正文上限为 4/16/64 MiB、active capability 为 4,096/16,384/65,536，绝对寿命为 8 小时/24 小时/7 天；结构化解析还取 8 MiB 输入、16 MiB 输出硬上限和 profile 上限中的最小值，并预留输入、对象树与输出的完整工作集。全进程还限制 16,384 authority、131,072 capability、256 MiB capability registry、1,024 动态流、每分钟 2,400 新 authority、32 DNS worker、8 并发解析和 256 MiB 解析内存；每站同时最多解析 2 个响应、capability registry 与解析内存各最多 64 MiB
- Safe 启用时至少需要一条经 IDNA/公共后缀规范化的 exact/suffix DNS 规则且不允许 IP literal；Compatible/Extreme 只允许公网目标。新建站点默认允许 HTTPS→HTTP 降级，以兼容只提供 HTTP 播放地址的后端；Safe 不能通过该设置放宽。私网目标、自定义 CA、跳过 TLS 验证和 raw fallback 不可用
