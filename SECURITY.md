...
- Safe 启用时至少需要一条经 IDNA/公共后缀规范化的 exact/suffix DNS 规则且不允许 IP literal；Compatible/Extreme 只允许公网目标。HTTPS→HTTP 默认拒绝，Safe 不能通过开关放宽。私网目标、自定义 CA、跳过 TLS 验证和 raw fallback 不可用
